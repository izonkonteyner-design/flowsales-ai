...
const endpoints = [
  { method: "GET", path: "/api/v1/leads", purpose: "Tenant kapsamlı potansiyel müşteri listesi", auth: "Bearer API key · crm:read" },
  { method: "POST", path: "/api/v1/leads", purpose: "Idempotent potansiyel müşteri oluşturma", auth: "Bearer API key · crm:write" },
  { method: "GET", path: "/api/health", purpose: "Servis canlılık kontrolü", auth: "Public" },
  { method: "GET", path: "/api/integrations/meta/status", purpose: "Secret-free Meta yapılandırma kontrolü", auth: "Session + Owner/Admin" },
  { method: "GET", path: "/api/integrations/meta/connect", purpose: "Meta OAuth başlatma", auth: "Session + Owner/Admin" },
...
export default async function ApiLayerPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="API anahtarı oluştur" description="Ham anahtar yalnız oluşturma anında gösterilir; veritabanında yalnız SHA-256 hash tutulur."><ApiKeyCreator /></SectionCard>
      ...
      <SectionCard title="Public API kullanım kuralları" description="Anahtarlar tenant ve scope bazlıdır; istekler rate-limit edilir.">
        <div className="grid gap-3 md:grid-cols-3"><Rule title="Kimlik doğrulama" detail="Authorization: Bearer fsa_…" /><Rule title="Yazma güvenliği" detail="POST /api/v1/leads için Idempotency-Key zorunlu." /><Rule title="Secret politikası" detail="Ham anahtar yalnız bir kez gösterilir ve loglanmaz." /></div>
      </SectionCard>

      <SectionCard title="Doğrulanmış endpoint'ler" description="Uygulamada gerçekten bulunan temel platform endpoint'leri."><div className="space-y-3">{endpoints.map((endpoint) => <div key={`${endpoint.method}:${endpoint.path}`} className="flex flex-col gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] p-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><Braces className="h-4 w-4 text-cyan-300" /><div><p className="font-mono text-sm font-medium text-white">{endpoint.method} {endpoint.path}</p><p className="mt-1 text-sm text-slate-500">{endpoint.purpose}</p></div></div><span className="text-xs text-slate-500">{endpoint.auth}</span></div>)}</div></SectionCard>
    </div>
  );
}

function Rule({ title, detail }: { title: string; detail: string }) {
  return <div className="rounded-2xl border border-white/[0.08] bg-white/[0.035] p-4"><p className="text-sm font-semibold text-white">{title}</p><p className="mt-2 text-xs leading-5 text-slate-500">{detail}</p></div>;
}
