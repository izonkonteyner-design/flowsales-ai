  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <SectionCard title="Sales Forecasting" description="Satış aşaması ve Lead Score birlikte kullanılarak olasılık ağırlıklı tahmin hesaplanır.">
        ...
        <div className="space-y-2">{forecast.opportunities.slice(0,8).map((row)=><div key={row.leadId} className="grid gap-2 rounded-xl border border-white/10 p-3 sm:grid-cols-[1fr_auto_auto]"><div><Link href={`/leads/${row.leadId}/quote-intelligence`} className="font-medium text-white hover:underline">{row.name}</Link><p className="text-xs text-slate-500">{stageLabels[row.stage] || row.stage} · Score {row.score}</p></div><StatusBadge tone={row.probability>=70?"success":row.probability>=40?"warning":"neutral"}>%{row.probability}</StatusBadge><span className="text-sm text-slate-300">{formatCurrency(row.weightedValue,"TRY")}</span></div>)}</div>
      </SectionCard>
...
function Action({href,icon:Icon,label}:{href:string;icon:typeof Trophy;label:string}) { return <Link href={href} className="inline-flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm font-medium text-white hover:bg-white/[0.07]"><span className="inline-flex items-center gap-2"><Icon className="h-4 w-4" />{label}</span><ArrowUpRight className="h-4 w-4" /></Link>; }
