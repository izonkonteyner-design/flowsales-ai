  ...
  return (
    <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-200/70 bg-amber-50 px-5 py-4 dark:border-amber-800/40 dark:bg-amber-950/20">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-amber-700 dark:text-amber-400">
              Integration connections are disabled in the demo workspace. Sign up for a live workspace to connect your channels.
            </p>
          </div>
        </div>
      )}
      ...
      {!isDemo && canManage && whatsappConfigured && (
        <section className="mb-6 rounded-2xl border border-emerald-200/70 bg-emerald-50/70 p-5 dark:border-emerald-800/40 dark:bg-emerald-950/15" aria-label="WhatsApp self-service onboarding">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-semibold text-emerald-900 dark:text-emerald-200">WhatsApp self-service setup</h2>
              <p className="mt-1 text-sm text-emerald-800/80 dark:text-emerald-300/80">
                Complete setup here without manually copying access tokens into FlowSales.
              </p>
            </div>
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${whatsappConnected ? "bg-emerald-600 text-white" : "bg-white text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"}`}>
              {whatsappConnected ? "Connected" : "Ready to connect"}
            </span>
          </div>
      ...
      <p className="mt-8 text-center text-xs text-slate-400 dark:text-slate-600">
        WhatsApp, Instagram, and Facebook integrations are powered by the Meta Platform. Google integration uses the Google Business Profile API. TikTok integration uses TikTok for Business.
      </p>
    </div>
  );
}
