...
export default async function IntegrationsPage() {
  ...
  const metaMessagingConfigured = Boolean(metaAppId && process.env.META_APP_SECRET?.trim() && process.env.TOKEN_ENCRYPTION_KEY?.trim());
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold text-slate-950 dark:text-white">Integrations</h1><p className="text-sm text-slate-500 dark:text-slate-400">Connect your social and messaging channels to FlowSales AI.</p></div>
        </div>
      </div>
      ...
      {!isDemo && canManage && !anyConfigured && (
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-blue-200/70 bg-blue-50 px-5 py-4 dark:border-blue-800/40 dark:bg-blue-950/20">
          ...
          <div><p className="font-medium text-blue-800 dark:text-blue-300">Provider credentials not configured</p><p className="mt-0.5 text-sm text-blue-700 dark:text-blue-400">Configure the provider credentials in the production environment before connecting a channel.</p></div>
        </div>
      )}

      {!isDemo && canManage && whatsappConfigured && (
        <section className="mb-6 rounded-2xl border border-emerald-200/70 bg-emerald-50/70 p-5 dark:border-emerald-800/40 dark:bg-emerald-950/15" aria-label="WhatsApp self-service onboarding">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="font-semibold text-emerald-900 dark:text-emerald-200">WhatsApp self-service setup</h2><p className="mt-1 text-sm text-emerald-800/80 dark:text-emerald-300/80">Complete setup here without manually copying access tokens into FlowSales.</p></div>
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${whatsappConnected ? "bg-emerald-600 text-white" : "bg-white text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"}`}>{whatsappConnected ? "Connected" : "Ready to connect"}</span>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {["Open Meta Embedded Signup and choose the correct WABA + phone number.", "FlowSales encrypts the returned token and subscribes the WABA webhook.", "Run Health Check; missing webhook subscription is automatically repaired when possible."].map((step, index) => (
              <div key={step} className="flex gap-2 rounded-xl bg-white/70 p-3 text-xs text-slate-700 dark:bg-white/5 dark:text-slate-300">
                {whatsappConnected || index === 0 ? <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /> : <ArrowRight className="h-4 w-4 shrink-0 text-emerald-600" />}
      ...
      {!isDemo && canManage && metaMessagingConfigured && (
        <section className="mb-6 rounded-2xl border border-blue-200/70 bg-blue-50/70 p-5 dark:border-blue-800/40 dark:bg-blue-950/15" aria-label="Meta messaging onboarding">
          <h2 className="font-semibold text-blue-900 dark:text-blue-200">Instagram & Messenger self-service setup</h2>
          <p className="mt-1 text-sm text-blue-800/80 dark:text-blue-300/80">Connect with Meta OAuth, then explicitly select the Facebook Page or linked Instagram professional account. FlowSales encrypts credentials and subscribes messaging webhooks server-side.</p>
        </section>
      )}
