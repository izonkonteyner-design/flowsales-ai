...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    const rl = await checkRateLimit(`${ctx.organizationId}_${ctx.userId}_${clientIp}`, "facebook_disconnect", 5, 600000);
    ...
    await disconnectMetaMessaging({ organizationId: ctx.organizationId, userId: ctx.userId, provider: "facebook", connectionId });
    return Response.redirect(new URL("/settings/integrations?disconnected=facebook", request.url), 303);
  } catch (error) {
    ...
    return handleOAuthRouteError(error, { provider: "facebook", route: "/api/integrations/facebook/disconnect" });
  }
}
