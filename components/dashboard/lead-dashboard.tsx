...
import type { LeadDashboardMetrics } from "@/server/services/lead-domain";
...
export function LeadDashboard({ metrics, currency }: LeadDashboardProps) {
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Overview"
        title="Dashboard"
        description="Track your pipeline, follow-ups, and lead momentum from one command center."
        actions={
          <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
        <SectionCard
          title="Revenue trend"
          description="Lead revenue by month with live Supabase metrics."
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">
                {formatCurrency(metrics.wonRevenue, currency)}
              </p>
              <p className="mt-2 text-sm text-emerald-600 dark:text-emerald-400">Live lead revenue</p>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
              <p className="text-sm text-slate-500">Average deal size</p>
              <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">
                {formatCurrency(metrics.averageDealValue, currency)}
              </p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">
                {metrics.followUpsDue + metrics.overdueFollowUps}
              </p>
              <p className="mt-2 text-sm text-amber-600 dark:text-amber-400">{metrics.overdueFollowUps} overdue</p>
            </div>
