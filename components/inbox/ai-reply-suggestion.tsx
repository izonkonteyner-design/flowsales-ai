  ...
  async function generate() {
    ...
    try {
      ...
      if (!response.ok || !data?.suggestion) {
        throw new Error(data?.message || "AI reply suggestion could not be generated.");
      }
      setSuggestion(String(data.suggestion));
    } catch (err) {
      setError(err instanceof Error ? err.message : "AI reply suggestion could not be generated.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="border-b border-slate-800/80 bg-violet-950/20 px-6 py-2.5 text-xs">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="font-semibold text-slate-200">AI Reply Draft</span>
      ...
      {suggestion && (
        <div className="mt-2 rounded-xl border border-violet-800/50 bg-slate-950/80 p-3">
          <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-200">{suggestion}</p>
          <div className="mt-2 flex items-center justify-end gap-2">
            <button type="button" onClick={() => setSuggestion(null)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-slate-400 hover:bg-slate-800">
            ...
            <button type="button" onClick={() => void copySuggestion()} className="inline-flex items-center gap-1 rounded-lg bg-violet-600 px-2.5 py-1 font-semibold text-white hover:bg-violet-500">
              ...
              {copied ? "Copied — review before sending" : "Copy for review"}
            </button>
          </div>
        </div>
      )}
