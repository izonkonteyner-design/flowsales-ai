      ...
      <div className="p-4 border-b border-slate-800/80 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-sm font-bold text-slate-100 uppercase tracking-wider">Omnichannel Inbox</h2>
          </div>
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search contact, snippet, ID..." value={searchQuery}
        ...
        <div className="flex items-center space-x-1 overflow-x-auto no-scrollbar pt-1">
          {statusTabs.map((tab) => (
            <button key={tab.key} onClick={() => onStatusFilterChange(tab.key)}
              className={`px-2.5 py-1 text-[11px] font-medium rounded-lg whitespace-nowrap transition-colors ${statusFilter === tab.key ? "bg-emerald-600 text-white font-semibold" : "text-slate-400 hover:text-slate-200 hover:bg-slate-900"}`}>
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {isDemo ? (
          <div className="flex flex-col items-center justify-center p-8 text-center text-slate-400">
            ...
            <p className="text-[11px] text-slate-400 mt-1">Demo accounts cannot access live production conversations.</p>
          </div>
        ) : isLoading ? (
          <div className="space-y-3 p-2">{[1,2,3,4].map((i) => <div key={i} className="h-20 rounded-xl bg-slate-900/40 border border-slate-800/40 animate-pulse" />)}</div>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center text-slate-500">
            ...
            <p className="text-[11px] text-slate-400 mt-1">{searchQuery ? "Try clearing search filter" : "Incoming WhatsApp messages will populate this list."}</p>
          </div>
        ) : (
          <>
            {visibleConversations.map((conv) => <ConversationItem key={conv.id} conversation={conv} isSelected={selectedId === conv.id} onSelect={onSelect} />)}
            {hasMore && (
              <button onClick={() => setVisibleCount((count) => Math.min(count + PAGE_SIZE, conversations.length))}
                className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg border border-slate-800 bg-slate-900/70 py-2 text-[11px] font-medium text-slate-400 hover:text-slate-200">
                <ChevronDown className="h-3.5 w-3.5" /> Load {Math.min(PAGE_SIZE, conversations.length - visibleCount)} more
              </button>
            )}
          </>
        )}
