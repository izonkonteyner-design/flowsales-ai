...
export function CrmConversationActions({ conversationId, disabled, onChanged }: {
  ...
  const addNote = async () => {
    const note = window.prompt("Add a CRM note from this WhatsApp conversation:");
    if (note?.trim()) await post({ action: "add_note", note: note.trim() });
  };

  const createFollowUp = async () => {
    const title = window.prompt("Follow-up task title:", "Follow up on WhatsApp conversation");
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-slate-800/70 bg-slate-950/80 px-4 py-2">
      ...
      <button disabled={disabled || busy} onClick={addNote} className="rounded-md border border-slate-700 px-2 py-1 text-[11px] text-slate-200 disabled:opacity-50">
        <ClipboardPlus className="mr-1 inline h-3 w-3" /> Note
      </button>
      <button disabled={disabled || busy} onClick={createFollowUp} className="rounded-md border border-slate-700 px-2 py-1 text-[11px] text-slate-200 disabled:opacity-50">
        <CheckCircle2 className="mr-1 inline h-3 w-3" /> Follow-up
      </button>
      <button disabled={disabled || busy || !identity?.lead} onClick={convert} className="rounded-md border border-slate-700 px-2 py-1 text-[11px] text-slate-200 disabled:opacity-50">
        <UserRoundCheck className="mr-1 inline h-3 w-3" /> Convert
      </button>
      <button disabled={disabled || busy || !identity?.lead} onClick={openQuote} className="rounded-md border border-slate-700 px-2 py-1 text-[11px] text-slate-200 disabled:opacity-50">
        <FilePlus2 className="mr-1 inline h-3 w-3" /> Create quote
      </button>
