...
export function CrmIdentityPanel({ conversationId, provider = "whatsapp", isReadOnly, onChanged }: Props) {
  ...
  async function mutate(body: Record<string, unknown>) {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "CRM identity update failed."); }
  ...
  async function searchExisting() {
    ...
    try {
      const res = await fetch(`/api/inbox/conversations/${conversationId}/identity?q=${encodeURIComponent(searchQuery.trim())}`, { cache: "no-store" });
      if (!res.ok) throw new Error("CRM search failed.");
      setSearchResults(await res.json());
    } catch (err) { setError(err instanceof Error ? err.message : "CRM search failed."); }
    finally { setSearching(false); }
  }

  if (!conversationId) return null;
  if (loading && !identity) return <div className="flex items-center gap-2 border-b border-slate-800 bg-slate-900/70 px-6 py-2 text-xs text-slate-400"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Resolving CRM identity…</div>;
  if (!identity) return error ? <div className="border-b border-rose-900/50 bg-rose-950/40 px-6 py-2 text-xs text-rose-300">{error}</div> : null;
