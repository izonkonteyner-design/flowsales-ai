...
export function InboxShell({ initialConversationId }: InboxShellProps) {
  ...
  useEffect(() => {
    ...
    async function loadConversations() {
      ...
      try {
        ...
        if (isMounted) console.error("Failed to load inbox conversations:", err);
  ...
  useEffect(() => {
    ...
    async function loadDetail() {
      if (!selectedId) {
        if (isMounted) { setActiveConversation(null); setIsDetailLoading(false); }
        return;
      }
  ...
  const handleRefresh = () => {
    ...
    if (selectedId) {
      fetchConversationDetailAction(selectedId).then((res) => setActiveConversation(res.conversation));
    }
