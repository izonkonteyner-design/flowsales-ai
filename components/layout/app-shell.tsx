...
const navigationGroups = [
  ...
  { key: "intelligence" as const, matches: ["/ai", "/ai-history", "/approvals", "/reports"] },
          ...
          <div key={group.key}>
            ...
            <div className="space-y-1">
              {items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={onNavigate} title={collapsed ? label : undefined}
    ...
    <div className="min-h-screen bg-[#050816] text-white">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1800px]">
        <aside className={cn("sticky top-0 hidden h-screen shrink-0 border-r border-white/[0.08] bg-[#080c19]/92 backdrop-blur-2xl xl:flex xl:flex-col", isSidebarCollapsed ? "w-[88px]" : "w-[286px]")}> 
          <div className="flex h-20 items-center justify-between border-b border-white/[0.08] px-4">
            <Link href="/dashboard" className="flex min-w-0 items-center gap-3">
              ...
              {!isSidebarCollapsed ? <div className="min-w-0"><p className="truncate text-sm font-semibold">{BRAND.name}</p><p className="truncate text-[11px] text-slate-500">{t(locale, "aiRevenueWorkspace")}</p></div> : null}
            </Link>
            <button type="button" onClick={() => setIsSidebarCollapsed((value) => !value)} className="rounded-xl p-2 text-slate-500 hover:bg-white/[0.06] hover:text-white" aria-label={t(locale, "toggleSidebar")}>{isSidebarCollapsed ? <PanelLeftOpen className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}</button>
          </div>
          ...
          <div className="border-t border-white/[0.08] p-3">
            {!isSidebarCollapsed ? <Link href="/usage" className="mb-3 block rounded-2xl border border-violet-400/15 bg-gradient-to-br from-violet-500/10 via-blue-500/5 to-transparent p-3 transition hover:bg-white/[0.05]"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-xs font-medium text-violet-200"><Zap className="h-3.5 w-3.5" /> {t(locale, "aiUsage")}</span><span className="text-[11px] text-slate-500">{t(locale, "view")}</span></div></Link> : null}
            <Link href="/account" className={cn("flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] p-3 hover:bg-white/[0.06]", isSidebarCollapsed && "justify-center")}><UserCircle2 className="h-9 w-9 text-slate-400" />{!isSidebarCollapsed ? <div className="min-w-0"><p className="truncate text-sm font-medium">{workspace.mode === "demo" ? t(locale, "demoUser") : workspaceName}</p><p className="truncate text-xs text-slate-500">{t(locale, "viewAccount")}</p></div> : null}</Link>
          </div>
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 border-b border-white/[0.08] bg-[#060a16]/75 backdrop-blur-2xl"><div className="flex h-20 items-center gap-3 px-4 sm:px-6 lg:px-8">
            <button type="button" className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.04] text-slate-300 xl:hidden" onClick={() => setIsMobileNavOpen(true)} aria-label={t(locale, "openNavigation")}><Menu className="h-4 w-4" /></button>
            <form action="/leads" method="get" className="relative hidden max-w-xl flex-1 lg:flex"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input name="search" type="search" placeholder={t(locale, "searchPlaceholder")} className="h-11 w-full rounded-2xl border border-white/[0.08] bg-white/[0.035] pl-10 pr-20 text-sm text-white outline-none placeholder:text-slate-600 focus:border-violet-400/40 focus:ring-4 focus:ring-violet-500/10" /><button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl border border-white/[0.08] bg-white/[0.05] px-3 py-1.5 text-xs text-slate-300 hover:bg-white/[0.1]">{t(locale, "search")}</button></form>
