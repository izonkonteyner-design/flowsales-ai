...
export function QuoteDeleteDialog({ quoteId, quoteName, redirectTo, recordMode, role }: QuoteDeleteDialogProps) {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="quote-delete-title">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_30px_120px_rgba(15,23,42,0.35)] dark:border-white/10 dark:bg-slate-950">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p id="quote-delete-title" className="text-lg font-semibold text-slate-950 dark:text-white">
                  Delete quote
                </p>
                <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
                  This will permanently remove <span className="font-medium text-slate-950 dark:text-white">{quoteName}</span> from the current organization.
                </p>
              </div>
              <button
                type="button"
            ...
            <form action={deleteQuoteAction} className="mt-6 flex flex-wrap gap-3">
