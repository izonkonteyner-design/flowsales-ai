...
export type WhatsAppAiSuggestionResult =
  | { success: true; suggestion: string; provider: "whatsapp" | "instagram" | "facebook" }
...
export async function generateWhatsAppReplySuggestion(params: {
  ...
  if (params.userRole === "viewer") return { success: false, errorCode: "unauthorized", message: "Read-only users cannot generate AI reply suggestions." };
  if (!hasGeminiConfig()) return { success: false, errorCode: "ai_not_configured", message: "AI reply suggestions are not configured." };
  ...
  if (!conversation || !["whatsapp", "instagram", "facebook"].includes(conversation.provider)) {
    return { success: false, errorCode: "not_found", message: "Supported messaging conversation not found." };
  }
  ...
  const prompt = [
    `You are drafting a ${channelLabel} reply for a human sales agent.`,
    ...
    "Conversation:", transcript || "[No usable message text]",
  ].join("\n");

  try {
    const text = (await generateText(prompt, { temperature: 0.3 })).trim().slice(0, MAX_SUGGESTION_CHARS);
    if (!text) return { success: false, errorCode: "empty_suggestion", message: "AI returned an empty suggestion." };
    return { success: true, suggestion: text, provider };
  } catch {
