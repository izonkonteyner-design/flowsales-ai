...
import { normalizePhoneNumberToE164, ALLOWLISTED_TEST_RECIPIENT_CANONICAL } from "@/lib/utils/test-recipient-guard";
...
export class WhatsAppTemplateVerificationService {
  async syncTemplateCatalog(organizationId: string) {
    ...
    if (!connection?.id || connection.status !== "connected" || !connection.waba_id) return { success: false as const, error: "connection_required" };
    ...
    const json = await response.json().catch(() => null) as { data?: Array<Record<string, unknown>>; error?: { code?: number } } | null;
  ...
  async verifyApprovedDelivery(params: { organizationId: string; userId: string; userRole: string }) {
    ...
    const supabase = createSupabaseAdminClient();
    const { data: conversations } = await supabase.from("conversations").select("id,external_id").eq("organization_id", params.organizationId).eq("provider", "whatsapp");
    const testConversation = (conversations || []).find((row) => normalizePhoneNumberToE164(row.external_id || "") === ALLOWLISTED_TEST_RECIPIENT_CANONICAL);
    ...
    await supabase.from("messages").update({ metadata: {
      ...
      verification_recipient: ALLOWLISTED_TEST_RECIPIENT_CANONICAL, verification_type: "controlled_test",
