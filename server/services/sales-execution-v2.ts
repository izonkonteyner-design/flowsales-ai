...
const STAGE_WEIGHT: Record<string, number> = { negotiation: 25, quote_sent: 22, quote_ready: 20, qualified: 15, discovery: 8, new_lead: 5, support: 0, won: -100, lost: -100 };
...
export async function listSalesExecutionPriorities(params: { organizationId: string; userId: string; userRole: string; limit?: number }): Promise<SalesExecutionPriority[]> {
  const { organizationId, userId, userRole } = params;
  const limit = params.limit ?? 10;
  const admin = createSupabaseAdminClient();
  const { data: qualifications, error } = await admin.from("conversation_ai_qualifications")
    ...
    .eq("organization_id", organizationId).in("status", ["suggested", "accepted"]).order("created_at", { ascending: false }).limit(250);
  ...
  const candidateRows = [...latestByConversation.values()].filter((row) => !["won", "lost", "support"].includes(row.sales_stage || ""));
  ...
  const rows = candidateRows.filter((row) => conversationMap.has(row.conversation_id));
  ...
  for (const action of actions || []) if (!earliestAction.has(action.conversation_id) && conversationMap.has(action.conversation_id)) earliestAction.set(action.conversation_id, action.scheduled_for);
  ...
  return rows.map((row) => {
    ...
    const rankScore = Math.max(0, row.score || 0) + PRIORITY_WEIGHT[priority] + (STAGE_WEIGHT[row.sales_stage || "new_lead"] || 0) + Math.min(30, overdue * 2) + (row.status === "accepted" ? 5 : 0);
    return {
...
export async function applyQualificationToLead(params: { organizationId: string; userId: string; userRole: string; conversationId: string; qualificationId: string }) {
  ...
  const { error } = await admin.from("leads").update({ notes, city, updated_at: new Date().toISOString() }).eq("organization_id", params.organizationId).eq("id", qualification.lead_id);
  if (error) throw new Error("CRM önerileri lead kaydına uygulanamadı.");

  const appliedFields = ["notes", ...(city !== lead.city ? ["city"] : [])];
  await admin.from("omnichannel_audit_events").insert({ organization_id: params.organizationId, conversation_id: params.conversationId, actor_user_id: params.userId, event_type: "ai_crm_suggestion_applied", metadata: { qualification_id: qualification.id, lead_id: qualification.lead_id, fields: appliedFields } });
  return { leadId: qualification.lead_id, appliedFields };
}
