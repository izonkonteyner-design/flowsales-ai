...
export class WhatsAppCrmActionsService {
  ...
  private async conversation(organizationId: string, conversationId: string) {
    const { data, error } = await this.supabase
      .from("conversations")
      .select("id,organization_id,lead_id,customer_id,channel_contact_id")
  ...
  async addNote(input: { organizationId: string; conversationId: string; userId: string; note: string }) {
    ...
    if (conversation.lead_id) {
      const { error } = await this.supabase.from("activities").insert({
        ...
        type: "whatsapp_note",
        title: "WhatsApp conversation note",
        detail: note,
      ...
      const { error } = await this.supabase
        .from("contacts")
        .update({ notes: `${existing}[WhatsApp] ${note}`.slice(0, 10000) })
        .eq("organization_id", input.organizationId)
        .eq("id", conversation.customer_id);
    ...
    await recordWhatsAppAuditEvent({ organizationId: input.organizationId, conversationId: input.conversationId,
      actorUserId: input.userId, eventType: "crm_note_added", metadata: { length: note.length } });
    return { success: true };
  }
  ...
  async createTask(input: {
    ...
    await recordWhatsAppAuditEvent({ organizationId: input.organizationId, conversationId: input.conversationId,
      actorUserId: input.userId, eventType: "crm_task_created", metadata: { priority: input.priority, dueAt: due.toISOString() } });
    return { success: true };
  }
  ...
  async convertLeadToCustomer(input: { organizationId: string; conversationId: string; userId: string }) {
    ...
    await recordWhatsAppAuditEvent({ organizationId: input.organizationId, conversationId: input.conversationId,
      actorUserId: input.userId, eventType: "crm_lead_converted", metadata: { leadId: lead.id, customerId } });
  ...
  async recordQuoteOpened(input: { organizationId: string; conversationId: string; userId: string }) {
    ...
    await recordWhatsAppAuditEvent({ organizationId: input.organizationId, conversationId: input.conversationId,
      actorUserId: input.userId, eventType: "crm_quote_opened", metadata: { leadId: conversation.lead_id } });
