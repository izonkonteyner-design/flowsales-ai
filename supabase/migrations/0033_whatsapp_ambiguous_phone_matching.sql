-- Fail closed when normalized CRM phone matching is ambiguous.
...
begin
  ...
  select count(*), case when count(*) = 1 then min(id) else null end
    into v_crm_contact_matches, v_crm_contact_id
    from contacts
  ...
  values (
    p_organization_id, 'whatsapp', p_sender_external_id, p_sender_name, p_sender_external_id,
    v_crm_contact_id, v_lead_id,
    jsonb_build_object('crm_contact_match', case when v_crm_contact_matches > 1 then 'ambiguous' when v_crm_contact_matches = 1 then 'matched' else 'none' end,
...
declare
  ...
  v_required_version constant text := '0033';
  v_function text;
  v_table text;
begin
...
values ('0033', '0033_whatsapp_ambiguous_phone_matching.sql', '97f453ddb031b6b622c07fc9c83dca9e')
