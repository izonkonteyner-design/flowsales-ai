...
test("production migration workflow preserves Conversation Intelligence, identity guard and Voice Sales while advancing through 0050", async () => {
  ...
  assert.match(workflow, /Apply and verify migrations through 0050/);
  assert.match(workflow, /supabase\/migrations\/\*\*/);
  assert.match(workflow, /test "\$latest" = "0050"/);
