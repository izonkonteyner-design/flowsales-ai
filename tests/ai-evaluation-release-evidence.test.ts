...
test("production migration workflow preserves Conversation Intelligence, identity guard and Voice Sales while advancing through 0051", async () => {
  ...
  assert.match(workflow, /Apply and verify migrations through 0051/);
  assert.match(workflow, /supabase\/migrations\/\*\*/);
  assert.match(workflow, /test "\$latest" = "0051"/);
  assert.match(workflow, /0045/);
  assert.match(workflow, /0046/);
  assert.match(workflow, /0047/);
  assert.match(workflow, /0048/);
  assert.match(workflow, /0049/);
