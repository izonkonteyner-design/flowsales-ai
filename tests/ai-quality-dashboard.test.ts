...
test("AI quality RPC aggregates feedback, versions, models and evaluation evidence", async () => {
  const sql = await readFile(migration, "utf8");
  for (const contract of ["ai_run_feedback", "ai_evaluation_runs", "prompt_version", "helpful_rate", "feedback_coverage", "latest_evaluation_failed"]) assert.match(sql, new RegExp(contract, "i"));
...
test("AI quality response parser fails closed", () => {
  const parsed = aiQualityDashboardSchema.parse({
    ...
    summary: { windowDays: 30, completedRuns: 10, failedRuns: 1, feedbackCount: 5, helpfulCount: 4, notHelpfulCount: 1, feedbackCoverage: 0.5, helpfulRate: 0.8 },
    segments: [{ capability: "lead_scoring", prompt_version: "2026-08-01.1", model: "gemini-test", completed_runs: 10, failed_runs: 1, feedback_count: 5, helpful_count: 4, not_helpful_count: 1, helpful_rate: 0.8, latest_run_at: "2026-08-02T11:00:00.000Z" }],
    evaluations: [{ id: "11111111-1111-4111-8111-111111111111", suite_key: "ai-regression", prompt_version: "2026-08-01.1", model: "gemini-test", total_cases: 4, passed_cases: 4, score: 1, status: "passed", commit_sha: "abc123", created_at: "2026-08-02T10:00:00.000Z" }], risks: [],
  });
...
test("AI quality migration remains registered while deployment readiness advances to 0041", async () => {
  const sql = await readFile(migration, "utf8");
