...
test("ambiguous normalized phone matches remain unlinked", async () => {
  const sql = await readFile("supabase/migrations/0033_whatsapp_ambiguous_phone_matching.sql", "utf8");
  assert.match(sql, /case when count\(\*\) = 1 then min\(id\) else null end/i);
